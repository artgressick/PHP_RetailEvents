	...
	*/
	...
	if(count($_POST)) {
		...
		if(count($error_messages) == 0) {
			...
			if(do_mysql_query($q, 'insert store')) {
				...
				foreach($ids as $user_id) {
					if(!isset($info['Users'][$user_id])) {
						...
						do_mysql_query("UPDATE ACL SET enPermission='" . $_POST['enPermission' . $user_id] . "'
							WHERE enType='Stores' AND idItem='" . $newID . "' AND idUser='" . $user_id . "'
			...
			foreach($info['Users'] as $user_id => $user) {
				...
				$new_store = mysql_insert_id();
				
				audit_new_record('Stores', $new_store);
				
				$_SESSION['InfoMessage'][] = 'The new store <span class="Specific">' . $_POST['chrName'] . '</span> has been saved.';

				header("Location: index.php");
				die();
			} else {
				$error_messages[] = 'There was a problem creating the new store.';
			}
		}
...
<script type="text/javascript">//<![CDATA[
	...
	//]]></script>
		...
		<div class="AdminTopicHeader">Add Stores</div>
			...
			<tr>
				<td class="Left">
					...
					<div class='sectionInfo'>
						...
						<div class='form'>
							<div class='formHeader'>Name <span class='Required'>(Required)</span>
...
<?	} ?>
						...
						<div class='form'>
							<div class='formHeader'><input type='checkbox' name='bIgnore' <?=(isset($info['bIgnore']) ? 'checked' : '')?> /> Ignore Monthly Requirements </div>
						</div>
...
<?	} ?>
						...
						<div class='form'>
							...
							<div class='' style='padding-left: 2em; margin-top: 3px;'>
								<span id='intThMC' style='margin-left: 10px; display: none;'><input type='text' size='13' name='intTheaterMaxCap' value='<?=@$info['intTheaterMaxCap']?>' /><span class='Required'> (Max Cap)</span></span>
							</div>
						</div>
						<script type="text/javascript">//<![CDATA[
							if(document.forms[0].bDedicatedTheater.checked) 
...
// ]]></script>
		...
		<div class='FormButtons'>
			<input type='submit' name='SubmitAddStore' value='Save New Store' />
			</div>
