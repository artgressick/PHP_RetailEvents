<?php
	...
	*/
	...
	if(isset($_POST['chrPageTitle'])) {
		...
		list($mysqlStr,$audit) = set_strs($mysqlStr,'idSection',$info['idSection'],$audit,$table,$_POST['id']);
		...
		list($mysqlStr,$audit) = set_strs($mysqlStr,'bPassword',$info['bPassword'],$audit,$table,$_POST['id']);		
		list($mysqlStr,$audit) = set_strs($mysqlStr,'chrPassword',$info['chrPassword'],$audit,$table,$_POST['id']);		
	...
	if(!isset($_REQUEST['sortCol'])) { $_REQUEST['sortCol'] = "chrPageTitle"; }
...
<script language="javascript" type="text/javascript">	
	function error_check(addy) {
		...
		total += ErrorCheck('idSection', "You must pick a Section.");
...
?>
		...
		<div class="AdminTopicHeader">Edit Dynamic Page</div>
		...
		<table>
			<tr>
				<td>
					...
					<div>
						<select name='idSection' id='idSection'>
							...
							<?	while($row = mysqli_fetch_assoc($sections)) { ?>					
								<option <?= ($info['idSection'] == $row['ID']) ? 'selected' : '' ?> value='<?=$row['ID']?>'> <?=decode($row['chrSectionName'])?></option>
							<?	} ?>
						</select>
					</div>
					<br />
					...
					<div style='margin-bottom: 10px;'><input type="checkbox" name="bPassword" id="bPassword" <?=($info['bPassword'] == 1 ? "checked" : "")?> /> Password Protected</div>
					<div class='formHeader'>Password<span class="Required"> (Required if Password checkbox is checked)</span></div>
					<div style='margin-bottom: 10px;'>
						<input type='text' size='20' maxlength='20' name='chrPassword' id='chrPassword' value="<?=base64_decode($info['chrPassword'])?>" />
					</div>
		...
		<div class='FormButtons'>
			<input type='button' name='SubmitAddDynamic' value='Save Dynamic Page' onclick='error_check()' />
