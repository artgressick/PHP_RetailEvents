<?php
	...
	$query = "SELECT Events.*, DATE_FORMAT(dDate, '%e') AS intMonthDay, TIME_FORMAT(tBegin, '%H') AS intStartHour, bApproved, Events.intSeries, EventTypes.idEventCategory,
		TIME_FORMAT(tBegin, '%M0') AS intStartMinute, EventTypes.chrName AS chrEventTypeName, EventTypes.chrStyleClass,
...
<? if($chkClone == 0) { ?>
	<div style='margin-top: 3px;'> <input <?=((($disableAdd == 1) && $_SESSION['idType'] != 1) ? 'disabled' : '')?> type='button' value='Clone Calendar' onclick='location.href="cloneCOE.php?idStore=<?=$_REQUEST['idStore']?>&intDate=<?=($_REQUEST['intDate'] - 1)?>"' /> To clone last month's calendar click here.</div>
<?	} ?>							
	...
	<? }
		...
		?>
			<div class="<?=$row['chrStyleClass']?> info">			
				...
				<a href="editevent.php?id=<?=$row['ID']?>&idStore=<?=$_REQUEST['idStore']?>&intDate=<?=$_REQUEST['intDate']?>" title="<?=str_replace('"',"&quot;",stripslashes($row['chrDescription']))?>" class="<?=$row['chrStyleClass']?>"><?=stripslashes($row['chrTitle'])?></a>
...
$submitbutton = "SELECT chrEventTitle, count(Events.ID) as intCount
	FROM EventTypeNames
	LEFT JOIN Events ON Events.idEventTitle=EventTypeNames.ID AND Events.idStore='" . $_REQUEST['idStore'] . "' AND Events.dDate LIKE '" . $intYear . "-" . ($intMonth<10 ? '0'.$intMonth : $intMonth) . "-%'
	WHERE EventTypeNames.bWeeklyRequired=1 AND !EventTypeNames.bDeleted
...
// Lets grab the Ignore check boxes for this store from the DB
$ignoreChk = mysql_fetch_assoc(do_mysql_query("SELECT bIgnore, bIgnoreRecaps FROM Stores WHERE ID=".$_REQUEST['idStore'],"getting ignore status"));

// START NEW CODE

// Set some Variables we will use later
$showButton = true;  // true = show button, false = hide button
$meetscount = false;  // ture = meets count requirements, false = does not
$infomessages = array(); // Start array for messages that will appear if $showbutton is false
$messages = array(); // Start array for messages that will appear if $showbutton is true


// Check to see if the store needs to do recaps from 2 months ago
if ($ignoreChk['bIgnoreRecaps'] != 1) { // If they are not exempt from this check then lets do it.
	...
	$Recaps = do_mysql_query("SELECT Events.ID, Recaps.chrStatus
		FROM Events
		LEFT JOIN Recaps on Events.ID = Recaps.idEvent
		JOIN EventTypes on Events.idEventType = EventTypes.ID
		WHERE Events.dDate LIKE '". $intRYear ."-". ($intRMonth < 10 ? '0'.$intRMonth : $intRMonth) ."-%' AND Events.idStore = '". $_REQUEST['idStore'] ."' AND EventTypes.bEditorReview = '1'",
		'Getting Recaps Count for Store');
	...
	if ($RecapCount > 0) {
		...
		$showButton = false;
		$infomessages[] = "You must complete all Recaps from ".$RecapsDate." in order to submit this Calendar. Total Recaps Not Complete=".$RecapCount.".  <a href='recaps.php?idStore=".$_REQUEST['idStore']
						  ."&intDate=".($_REQUEST['intDate']-2)."'><strong>Click HERE to goto your Recaps Page</strong></a>.";

	}
}		

if(($storeInfo['enStoreSize'] == 'Mini' || $storeInfo['chrStoreSize'] == 'Mini') && $storeInfo['eventCount'] >= 0) { $meetscount = true; }
if(($storeInfo['enStoreSize'] == '30 foot' || $storeInfo['chrStoreSize'] == '30 Foot') && $storeInfo['eventCount'] >= 12) { $meetscount = true; }
if(($storeInfo['enStoreSize'] == '45 foot' || $storeInfo['chrStoreSize'] == '45 Foot') && $storeInfo['eventCount'] >= 15) { $meetscount = true; }
if(($storeInfo['enStoreSize'] == 'High Profile' || $storeInfo['chrStoreSize'] == 'High Profile') && $storeInfo['eventCount'] >= 25) { $meetscount = true; }

// Check to see if we requiring the other monthly requirements
if($ignoreChk['bIgnore'] != 1) {  // If they are required then do this
	...
	if($monthChk['enStatus'] != 'Submitted' && $monthChk['enStatus'] != 'Approved') { // if month has not been submitted and not approved

		if($storeInfo['enStoreSize'] == 'Mini' || $storeInfo['chrStoreSize'] == 'Mini') {  // If Store is a mini Store
			$ok = 1;
			$size = 'Mini';
		} else {
		...
		if($edit_mode && $is_my_store && ($_REQUEST['intDate'] == ($current_month) || $_REQUEST['intDate'] == ($current_month + 1) || $_REQUEST['intDate'] == ($current_month + 2)) && $showButton=="Approved" && $ok == 1) { 
			...
			if($is_my_store) {
				...
				if($edit_mode) { 
					...
					if($showButton!=true) { 
						...
						if($storeInfo['enStoreSize'] == '' && $storeInfo['chrStoreSize'] == '') {
							...
							$showButton = false;
							$infomessages[] = "You must supply this store with a Store Size.";
							
	 					} else if($storeInfo['eventCount'] > 0) {
	
							$showButton = false;
							$infomessages[] = "The minimum number of workshops needed for your store size has not been met.";
						
						}
	
					} else if(mysql_num_rows($submitbutton_results) != 0 && $size != 'Mini') {				
	
						$temp = "At least one of each of the following Workshops must be created before you may submit this calendar<br /> -- ";
												
								$count=0;
							
								while($eventName = mysql_fetch_assoc($submitbutton_results)) { 
								
									$temp .= ($count>0 ? ", " : "") . '"'.$eventName['chrEventTitle'].'"';
						...
						$showButton = false;  
						$infomessages[] = "You may only submit a calendar when it is within the next one or two months of the current date.";
						
					} else { 
	
						$messages[] = 'When you are done making changes to your workshops/events calendar, click the "'.($monthChk['enStatus'] == 'Rejected' ? 'Re-Submit Calendar' : 'Submit Calendar').'" button to send it to the Retail Workshops/Events team.';
					...
					$showButton = false;  
					$infomessages[] = "Your month calendar has been submitted to the Retail Workshops/Events team for approval.  The calendar cannot be changed.  You will need to contact <a href='mailto:retailevents@apple.com'>retailevents@apple.com</a> with your change request.";
				...
				if($editChk['enStatus'] == 'Submitted') { 
					...
					$showButton = false;  
					$infomessages[] = "This workshop/event calendar has been submitted and is awaiting approval.";
					
				} else if($editChk['enStatus'] == 'Approved') {
	
					$showButton = false;  
					$infomessages[] = "This workshop/event calendar has been approved by Retail Workshops/Events.";
			...
			if($approval_mode) {
				...
				$messages[] = "You may make any changes, then click the \"Approve Calendar\" button above to finalize this store's calendar.";
					
			} else if($is_retailevents_user && $edit_mode && $this_month_status != 'Approved') {
	
				$messages[] = "This calendar has not yet been submitted, you can still make changes to it.";
					
			} else if($is_retailevents_user) {
	
				$messages[] = "Though it has been approved, you may make changes to it."
...
?>
	<div style='background-color: #C2CED8;'>
...
} else { // they need to meet some requirements


}  ?>
<?	if($monthChk['enStatus'] == '' && ($_SESSION['idType'] == 1 || in_array($_REQUEST['idStore'],$_SESSION['intStoreList']))) { ?>
		<div style='background-color: #C2CED8; padding: 3px; margin-top: 5px;'><input type='button' onclick='javascript:warning("1","1");' value='Clear All Workshops' /> This will delete all events for this month only.  <span style='color: red; background: black;'>&nbsp;! USE WITH CAUTION !&nbsp;</span></div>
<?	} ?>
